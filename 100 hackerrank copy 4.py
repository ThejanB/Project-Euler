# correct cases - 3,4,6,11,15,16,17
# other cases - wrong
...
def solve_one_case(P, Q, Dmin):
    ...
    target = Q - P  # right side
    X0, Y0 = None, None
    limit_initial_search = 200000  # can adjust as needed
    found_initial = False
    
    for m in range(1, limit_initial_search+1):
        lhs = Q*(m*m) - target  # = Q*m^2 - (Q-P) = Q*m^2 - Q + P = Q*(m^2 -1) + P
        if lhs < 0:
            continue
        if lhs % P != 0:
            continue
